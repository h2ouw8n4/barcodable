...
module Barcodable.com.Core where
...
-- | display the config
instance P.Show comConfig where
  show c =
    T.printf
...
-- | constructs a default comConfig
--
-- configHost:
--
...
-- configUserAgent:
--
-- @"barcodable.com/0.1.0.0"@
--
newConfig :: IO comConfig
newConfig = do
    logCxt <- initLogContext
    return $ comConfig
        { configHost = "https://virtserver.swaggerhub.com/magicCashew/barcodable/1.0.0"
        , configUserAgent = "barcodable.com/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => comConfig -> auth -> comConfig
addAuthMethod config@comConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: comConfig -> IO comConfig
...
withStderrLogging :: comConfig -> IO comConfig
...
withNoLogging :: comConfig -> comConfig
...
data comRequest req contentType res accept = comRequest
  { rMethod  :: NH.Method   -- ^ Method of comRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of comRequest
  , rParams   :: Params -- ^ params of comRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (comRequest req contentType res accept) NH.Method
rMethodL f comRequest{..} = (\rMethod -> comRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (comRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f comRequest{..} = (\rUrlPath -> comRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (comRequest req contentType res accept) Params
rParamsL f comRequest{..} = (\rParams -> comRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (comRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f comRequest{..} = (\rAuthTypes -> comRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => comRequest req contentType res accept -> param -> comRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: comRequest req contentType res accept -> param -> comRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: comRequest req contentType res accept -> param -> comRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
_mkRequest :: NH.Method -- ^ Method 
          ...
          -> comRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = comRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader :: comRequest req contentType res accept -> [NH.Header] -> comRequest req contentType res accept
setHeader req header =
  req `removeHeader` P.fmap P.fst header &
  L.over (rParamsL . paramsHeadersL) (header P.++)

removeHeader :: comRequest req contentType res accept -> [NH.HeaderName] -> comRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => comRequest req contentType res accept -> comRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of 
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => comRequest req contentType res accept -> comRequest req contentType res accept
_setAcceptHeader req =
    case mimeType (P.Proxy :: P.Proxy accept) of 
        Just m -> req `setHeader` [("accept", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["accept"]

setQuery :: comRequest req contentType res accept -> [NH.QueryItem] -> comRequest req contentType res accept
setQuery req query = 
  req &
  L.over
...
addForm :: comRequest req contentType res accept -> WH.Form -> comRequest req contentType res accept
addForm req newform = 
...
_addMultiFormPart :: comRequest req contentType res accept -> NH.Part -> comRequest req contentType res accept
...
_setBodyBS :: comRequest req contentType res accept -> B.ByteString -> comRequest req contentType res accept
_setBodyBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: comRequest req contentType res accept -> BL.ByteString -> comRequest req contentType res accept
_setBodyLBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => comRequest req contentType res accept -> P.Proxy authMethod -> comRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    :: comConfig
    -> a
    -> comRequest req contentType res accept
    -> IO (comRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> comConfig
  -> IO (comRequest req contentType res accept)
_applyAuthMethods req config@(comConfig {configAuthMethods = as}) =
  foldlM go req as
  where
