...
async def test_get_item_by_asin(client):
    ...
    response = await client.request(
        method='GET',
        path='/mindviser/barcodable/1.0.0/api/v1/asin/{asin}'.format(asin='asin_example'),
        headers=headers,
        )
...
async def test_get_item_by_ean(client):
    ...
    response = await client.request(
        method='GET',
        path='/mindviser/barcodable/1.0.0/api/v1/ean/{ean}'.format(ean='ean_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
...
async def test_get_item_by_upc(client):
    ...
    response = await client.request(
        method='GET',
        path='/mindviser/barcodable/1.0.0/api/v1/upc/{upc}'.format(upc='upc_example'),
        headers=headers,
        )
    assert response.status == 200, 'Response body is : ' + (await response.read()).decode('utf-8')
