...
void OAIApiRouter::createApiHandlers() { 
    mOAIProductConversionApiHandler = QSharedPointer<OAIProductConversionApiHandler>::create();
    mOAIProductLookupApiHandler = QSharedPointer<OAIProductLookupApiHandler>::create();
}


void OAIApiRouter::setOAIProductConversionApiHandler(QSharedPointer<OAIProductConversionApiHandler> handler){
    mOAIProductConversionApiHandler = handler;
}
void OAIApiRouter::setOAIProductLookupApiHandler(QSharedPointer<OAIProductLookupApiHandler> handler){
    mOAIProductLookupApiHandler = handler;
}
    ...
    auto reqPath = QString("%1 %2").arg(fromQHttpEngineMethod(socket->method())).arg(socket->path()).toLower();
    {
        ...
        if ( reqPath.startsWith(completePath.leftRef( completePath.indexOf(QString("/{")))) ) {
            ...
            if ( match.hasMatch() ){
                ...
                auto reqObj = new OAIProductConversionApiRequest(socket, mOAIProductConversionApiHandler);
    ...
    }
    {
        ...
        if ( reqPath.startsWith(completePath.leftRef( completePath.indexOf(QString("/{")))) ) {
            ...
            if ( match.hasMatch() ){
                ...
                auto reqObj = new OAIProductLookupApiRequest(socket, mOAIProductLookupApiHandler);
