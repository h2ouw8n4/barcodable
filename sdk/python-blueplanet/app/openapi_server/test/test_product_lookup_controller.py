...
class TestProductLookupController(BaseTestCase):
    ...
    def test_get_item_by_asin(self):
        ...
        response = self.client.open(
            '/magicCashew/barcodable/1.0.0/api/v1/asin/{asin}'.format(asin='asin_example'),
            method='GET')
    ...
    def test_get_item_by_ean(self):
        ...
        """
        response = self.client.open(
            '/magicCashew/barcodable/1.0.0/api/v1/ean/{ean}'.format(ean='ean_example'),
            method='GET')
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
    ...
    def test_get_item_by_upc(self):
        ...
        """
        response = self.client.open(
            '/magicCashew/barcodable/1.0.0/api/v1/upc/{upc}'.format(upc='upc_example'),
            method='GET')
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
